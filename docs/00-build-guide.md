...
More depth: [Concepts](01-concepts.md) and [SDK and DomiSMP concepts](25-sdk-and-domismp-concepts.md).
...
**Went wrong for us.** The adapter first showed `169.254.79.25/16`. That is an APIPA "no DHCP answer" address, and it means sub-step 3 above hasn't been done yet. See [VMware and network](04-vmware-network.md#additional-screenshot-derived-network-history).
...
**Went wrong for us.** The installer printed `Failed unmounting cdrom.mount - /cdrom` and waited for the medium to be removed. Disconnecting the virtual CD fixes it. See [Ubuntu and storage](05-ubuntu-storage.md).
...
!!! tip
    Always run `hostname` before pasting a block of commands. In this build, a Red-preparation block was once run on Blue by mistake ([failure 11](14-failures-and-fixes.md#11-red-commands-accidentally-executed-on-blue)).
...
Details: [Red gateway](11-red-gateway.md).
...
Ownership moves to the `domibus` user in step 12. Details: [Ubuntu and storage](05-ubuntu-storage.md).
...
**Went wrong for us.** Without the temporary setting, the import failed with `ERROR 1419`. See [Database setup](08-database.md#import-failure-error-1419).
...
!!! warning "Add `allowPublicKeyRetrieval=true` from the start"
    Our first build didn't have it. Everything worked until the first **reboot**, when Domibus returned 404 with `Public Key Retrieval is not allowed`. In plain words: MySQL 8's password method needs either TLS or permission for the driver to fetch the server's public key. With `useSSL=false`, you must allow the second. See [failure 17](14-failures-and-fixes.md#17-systemd-active-but-domibus-returned-404).
...
**Went wrong for us.** Starting from another directory caused a FreeMarker warning. A leftover JVM caused confusion. Blue's admin account had to be recreated through the database. See [Domibus installation](07-domibus-installation.md) and [failures 2, 3 and 9](14-failures-and-fixes.md).
...
Details: [Cryptography and PMode](09-crypto-and-pmode.md).
...
**Do it** (all observed; full commands in [SOAP extraction and test commands](23-soap-extraction-and-test-commands.md)):
...
**Verify.** Both directions reach `ACKNOWLEDGED` on the sender and `RECEIVED` on the receiver, and the payload is retrieved. The proven IDs are in [Raw observed values](22-raw-observed-values.md#blue-red-message).
...
1. Create `/etc/systemd/system/domibus.service`. The full unit is in [systemd and reboot](13-systemd-and-reboot.md#unit-file).
2. Run:
...
**Verify** about 75 seconds after boot. Use the full checklist in the [Operations runbook](16-operations-runbook.md). The short version:
...
Details: [sdk-core inventory](26-sdk-core-inventory.md) and [storage](27-sdk-core-storage.md).
...
Then configure these four files. The full contents are in [DomiSMP configuration](31-domismp-configuration.md):

| File | Purpose |
|---|---|
| `/opt/domismp/bin/setenv.sh` | Java path, PID file, heap `-Xms1024m -Xmx2048m` ([Tomcat runtime](30-domismp-tomcat-runtime.md#explicit-java-settings)) |
...
1. Create `/etc/systemd/system/domismp.service`. Use `ExecStop=/opt/domismp/bin/shutdown.sh 30 -force`, because a leftover thread kept the JVM alive on shutdown. Full unit: [DomiSMP systemd](33-domismp-systemd-reboot.md#final-unit).
2. Run `sudo systemctl enable --now domismp`, then `sudo reboot`, then re-check HTTP 200 on `/smp/`.
3. Power off cleanly and take the VMware snapshot **`02-domismp-installed`**.

---

## 18. What's next

The generic DomiSMP is running, but it is **not yet configured for Swedish SDK**: no SDK domain, participants, SML/DNS or certificates. Continue with [Next SDK configuration](43-next-sdk-configuration.md).
...
Full values: [Known-good state (Blue/Red)](17-evidence-and-known-good-state.md) and [sdk-core known-good state](37-sdk-core-known-good-state.md).
