...
This timeline consolidates the retained milestones in order. For a hands-on walkthrough of the same order with download links, see [Build it step by step](../build-guide.md).

!!! info "Clocks"
    Times in brackets such as *(17:54 local)* come from screenshot filenames, in Windows local time (CEST, UTC+2). Times from logs or the database are UTC. See [Raw observed values](20-raw-values.md#important-timestamps).
...
| Snapshot | After |
|---|---|
| `00-Base-OS-Clone-Ready` (Blue) | step 5, base OS; Red cloned from it |
| `01-Network-Storage-Ready` (Blue, Red) | step 12, static IPs and `/data` |
| `02-Domibus-DB-Ready` (Blue, Red) | step 28, schema imported |
| `03-Blue-PMode-Ready` ×2 / `03-Red-PMode-Ready` | steps 41 / 49, PModes uploaded |
| `04-Blue-AS4-Working` / `04-Red-AS4-Working` | step 78, AS4 both ways |
| `05-Blue-Reboot-Safe` / `05-Red-Reboot-Safe` | steps 94 / 100, cold reboots passed |

Details: [VMware snapshots](../reference/snapshots.md).
...
A complete chronological screenshot index is available in [Screenshot evidence](22-screenshots.md). The archive materially improves the timeline by supplying visual checkpoints from VMware network creation at 17:54 through Domibus/admin diagnostics at 21:51.
